...
// Next.js keeps local development secrets in .env.local, while dotenv
// defaults to .env. Load .env.local explicitly and fall back to .env.
...
async function main() {
  ...
  for (const sub of subscriberDefs) {
    ...
    // Migration 0004 enforces one current subscription row per user.
    // Keep this explicit check-then-write so the seed remains compatible
    // with the same manual subscription semantics used by the admin tools.
