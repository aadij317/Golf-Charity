...
- **No column-level RLS on `winners`.** Subscribers can technically update
  `verification_status` / `payment_status` on their own winner row
  (needed so they can set `proof_url`), because true column-level
  security needs a dedicated RPC. The proof-upload flow only ever writes
  `proof_url` and resets `verification_status` to `pending`, and the
  admin actions (`app/admin/winners/actions.ts`) validate transitions
  (e.g. `payment_status` can't move to `paid` unless
  `verification_status` is already `approved`) — but that's app-layer
  discipline, not a DB-level guarantee. Worth hardening with a
  `submit_winner_proof()` RPC beyond a trainee assignment.
...
created by the numbered migrations in `supabase/migrations/` — run all
three, in order, against your Supabase project before `npm run seed`.
...
Verified against a local build (`npm run build`):
...
- ✓ Draw simulation vs. publish — simulate never writes; publish is
  disabled until a simulation has succeeded; a re-publish of the same
  month/type surfaces the backend's `409` as "already published"; prize
  pool splits 40/35/25 with 5-match-only jackpot rollover
