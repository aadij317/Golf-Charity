    ...
    <div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={`$${totalPrizesPaid.toLocaleString()}`}
          sub={`$${totalPrizesPending.toLocaleString()} pending payout`}
        />
        <StatCard
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="panel p-5">
          <h2 className="mb-4 font-display text-lg italic text-paper">Winners by tier</h2>
          <TierChart data={tierData} />
        </div>

        <div className="panel p-5">
          <h2 className="mb-4 font-display text-lg italic text-paper">Draw statistics</h2>
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between">
              <dt className="text-paper/60">Published</dt>
              <dd className="score-num text-paper">{drawsByStatus.published}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-paper/60">Simulated</dt>
              <dd className="score-num text-paper">{drawsByStatus.simulated}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-paper/60">Draft</dt>
              <dd className="score-num text-paper">{drawsByStatus.draft}</dd>
            </div>
          </dl>
        </div>
      </div>

      <p className="mt-6 max-w-2xl text-xs leading-relaxed text-paper/40">
