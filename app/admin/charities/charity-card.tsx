  ...
  return (
    <div className="panel overflow-hidden">
      ...
      <div className="space-y-2 p-4">
        <h3 className="font-display text-lg italic text-ink">{charity.name}</h3>
        <p className="line-clamp-2 text-sm text-ink/60">{charity.description || "—"}</p>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2 text-xs">
          ...
          {confirmingDelete ? (
            <span className="flex items-center gap-2">
              <span className="text-ink/50">Delete?</span>
              <button onClick={handleDelete} disabled={isPending} className="text-flag underline underline-offset-2">
                Confirm
              </button>
              <button onClick={() => setConfirmingDelete(false)} className="text-ink/50 underline underline-offset-2">
                Cancel
              </button>
            </span>
          ) : (
            <button
              ...
              className="text-flag underline underline-offset-2"
            >
              Delete
            </button>
          )}
