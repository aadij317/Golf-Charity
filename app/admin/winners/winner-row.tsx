  ...
  return (
    <tr className="score-row align-top">
      <td className="px-4 py-3">
        <div className="text-ink">{winner.profiles?.full_name || winner.profiles?.email}</div>
      ...
      <td className="score-num px-4 py-3 text-ink">{winner.tier}-match</td>
      <td className="score-num px-4 py-3 text-ink">${winner.prize_amount}</td>
      <td className="px-4 py-3">
        {winner.signed_proof_url ? (
          ...
          <span className="text-xs text-ink/40">Proof uploaded (preview unavailable)</span>
        ) : (
          <span className="text-xs text-ink/40">No proof uploaded</span>
        )}
      </td>
      <td className="px-4 py-3">
        ...
        {winner.verification_status === "pending" && (
          <div className="mt-2 flex gap-2">
            <button onClick={() => verify("approved")} disabled={isPending} className="text-xs text-fairway underline underline-offset-2">
              Approve
            </button>
            <button onClick={() => verify("rejected")} disabled={isPending} className="text-xs text-flag underline underline-offset-2">
              Reject
            </button>
          </div>
        )}
      ...
      <td className="px-4 py-3">
        ...
        {winner.payment_status === "pending" ? (
          ...
          <button onClick={() => pay("pending")} disabled={isPending} className="mt-2 block text-xs text-ink/50 underline underline-offset-2">
            Revert to pending
          </button>
        )}
