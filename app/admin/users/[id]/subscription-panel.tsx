...
export default function SubscriptionPanel({
  ...
  return (
    <div className="panel space-y-4 p-5">
      <h2 className="font-display text-lg italic text-ink">Subscription</h2>

      {subscription ? (
        <dl className="grid grid-cols-2 gap-y-1 text-sm">
          <dt className="text-ink/50">Renews / ended</dt>
          <dd className="score-num text-ink">
            {subscription.current_period_end
              ? new Date(subscription.current_period_end).toLocaleDateString()
              : "—"}
          </dd>
          <dt className="text-ink/50">Charity share</dt>
          <dd className="score-num text-ink">{subscription.charity_contribution_pct}%</dd>
        </dl>
      ) : (
      ...
      <form action={handleSubmit} className="space-y-3 border-t border-line pt-4">
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="mb-1 block text-xs text-ink/60">Plan</label>
