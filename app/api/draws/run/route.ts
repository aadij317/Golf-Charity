 ...
 * rollover). Subscription fees are read from the configured Stripe Price
 * objects so simulations and published draws use the real configured amount.
 */
...
type RunBody = {
  ...
  // When publishing from a successful simulation, the UI sends the exact
  // previewed numbers back so the published result cannot silently change.
...
function isValidBody(b: any): b is RunBody {
  ...
  if (b.winning_numbers !== undefined) {
    if (b.mode !== "publish" || !Array.isArray(b.winning_numbers)) return false;
    if (b.winning_numbers.length !== NUMBERS_DRAWN) return false;
    const unique = new Set(b.winning_numbers);
    if (unique.size !== NUMBERS_DRAWN) return false;
    if (!b.winning_numbers.every((n: unknown) => typeof n === "number" && Number.isInteger(n) && n >= SCORE_MIN && n <= SCORE_MAX)) return false;
  }
  return true;
}
...
export async function POST(req: NextRequest) {
  ...
  const {
    ...
    winning_numbers: previewWinningNumbers,
  } = body;

  if (!isFirstDayOfMonth(month)) {
    return NextResponse.json({ error: "month must be the first day of a valid month (YYYY-MM-01)" }, { status: 400 });
  }
  ...
  const typedActiveSubs = (activeSubs ?? []) as ActiveSubscription[];
  const activeUserIds = typedActiveSubs.map((s) => s.user_id);
  ...
  if (previewWinningNumbers) {
    winningNumbers = [...previewWinningNumbers].sort((a, b) => a - b);
  } else if (draw_type === "random") {
  ...
  // --- 7. Prize pool math ---
  // Read the configured Stripe prices once per run. This avoids a hard-coded
  // fee and keeps monthly/yearly subscribers correctly represented.
  ...
  try {
    ...
    monthlyFee = Number(monthlyPrice.unit_amount ?? 0) / 100;
    yearlyFee = Number(yearlyPrice.unit_amount ?? 0) / 100;
  } catch (error: any) {
    return NextResponse.json({ error: error?.message ?? "Could not read Stripe prices" }, { status: 500 });
  }

  if (monthlyFee <= 0 || yearlyFee <= 0) {
    return NextResponse.json({ error: "Configured Stripe prices must have a positive unit amount" }, { status: 500 });
  }

  const subsByUser = new Map(typedActiveSubs.map((s) => [s.user_id, s]));
  let totalPool = 0;
  for (const userId of activeUserIds) {
    const sub = subsByUser.get(userId);
    const pct = Number(sub?.charity_contribution_pct ?? 10);
    const fee = sub?.plan === "yearly" ? yearlyFee / 12 : monthlyFee;
    totalPool += fee * (1 - pct / 100);
  }

  const { data: lastDraw, error: lastDrawError } = await admin
    .from("draws")
    ...
    .maybeSingle();
  if (lastDrawError) {
    return NextResponse.json({ error: lastDrawError.message }, { status: 500 });
  }
  const previousRollover = Number(lastDraw?.jackpot_rollover_amount ?? 0);
  ...
  if (drawErr || !draw) {
    if (drawErr?.code === "23505") {
      return NextResponse.json({ error: "A draw for this month/type already exists." }, { status: 409 });
    }
  ...
  if (entries.length) {
    ...
    if (entriesErr) {
      // Keep publish atomic from the user's perspective. Entries reference
      // the draw with ON DELETE CASCADE, so removing the just-created draw
      // also removes any partial entry rows before Stripe/email side effects.
      await admin.from("draws").delete().eq("id", draw.id);
      return NextResponse.json(
        { error: `Failed to create draw entries: ${entriesErr.message}` },
        { status: 500 }
      );
    }
  }
  ...
  if (winnerRows.length) {
    ...
    if (winnersErr) {
      // Winner rows also reference the draw with ON DELETE CASCADE. Roll the
      // whole publish back instead of leaving a published draw without its
      // corresponding winner records.
      await admin.from("draws").delete().eq("id", draw.id);
      return NextResponse.json(
        { error: `Failed to create winners: ${winnersErr.message}` },
        { status: 500 }
      );
    }
  }
    ...
    try {
      if (activeUserIds.length) {
        ...
        if (subscriberProfiles?.length) {
          await sendDrawPublishedEmail(
            ((subscriberProfiles ?? []) as Array<{ email: string; full_name: string | null }>).map((p) => ({ email: p.email, name: p.full_name })),
      ...
      if (winnerRows.length) {
        ...
        const profileById = new Map(
          ((winnerProfiles ?? []) as Array<{ id: string; email: string; full_name: string | null }>).map((p) => [p.id, p])
        );
