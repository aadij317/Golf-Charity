...
export default function ScoreRow({
  ...
  return (
    <li className="score-row flex items-center justify-between py-2">
      <span className="score-num text-paper">{score}</span>
      ...
      {!disabled && (
        <button
          ...
          disabled={deleting}
          className="text-xs text-flag-soft underline underline-offset-2 disabled:opacity-40"
        >
          {deleting ? "Removing…" : "Delete"}
        </button>
      )}
      {error && <span className="text-xs text-flag-soft">{error}</span>}
    </li>
  );
}
